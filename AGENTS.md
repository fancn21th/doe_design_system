...
- Compose foundation UI from `src/components/ui` shadcn primitives; check existing local usage before using `Select`, `DropdownMenu`, or other Base UI style primitives in a new way. Fumadocs styling is documentation-only and does not define DOE product UI.
